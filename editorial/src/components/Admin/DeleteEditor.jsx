...
function TableRow({ currentData }) {
  ...
  const handleEdit = async (id) => {
    try {
      const response = await axios.post(`http://localhost:3000/admin/removeEditor`, {
        id: id
      })
...
function DeleteEditor() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/admin/getEditor')
