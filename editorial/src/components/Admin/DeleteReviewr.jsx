...
function Nodata() {
  return (
    <div className="p-4  text-center ">
      <p className="mb-6">No reviewrs found.</p>
...
const navigateTo = useNavigate();
 const handleEdit = async (id) => {
  try{
    const response = await axios.post(`http://localhost:3000/admin/removeByid`,{
      id : id
    })
...
function DeleteReviewr() {
  const [data,setData]= useState ([]);
  const [loading,setLoading]= useState(true);
  useEffect(()=>{
    const fetchData = async () =>
   { try {
      const response = await axios.get('http://localhost:3000/admin/getReviewr')
