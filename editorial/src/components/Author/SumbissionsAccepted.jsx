...
<main onClick={(e) => e.stopPropagation()} className='mt-5 bg-indigo-100 shadow-md rounded-md w-1/2 py-5 px-10'>
        <h1 className="text-wrap whitespace-pre-wrap text-left text-2xl">{note}</h1>
      </main>
  )
}
...
function Nodata() {
  return (
    <div className="p-4  text-center ">
      <p className="mb-6">No Manuscripts Accepted yet</p>
...
function SubmissionsAccepted() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:3000/author/SubmissionAccepted");
...
export default SubmissionsAccepted;
