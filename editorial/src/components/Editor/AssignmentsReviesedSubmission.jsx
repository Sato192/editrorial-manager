...
function AssignmentsReviesedSubmission() {
  const [data,setData]= useState ([]);
  const [loading,setLoading]= useState(true);
  useEffect(()=>{
    const fetchData = async () =>
   { try {
      const response = await axios.get('http://localhost:3000/editor/revisedSubmissionsReqAssignmments')
...
export default AssignmentsReviesedSubmission ;